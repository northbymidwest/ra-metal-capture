...
./scripts/regen-libretro-bindings.sh        # after replacing third_party/libretro/libretro.h
```
...
- All Metal and AppKit calls go through `objc2-metal`, `objc2-foundation`,
  ...
  generated by `scripts/regen-libretro-bindings.sh` from the vendored
  `third_party/libretro/libretro.h` (RetroArch at the commit in
  `third_party/libretro/COMMIT`) with the
  `tools/regen-libretro-bindings` crate (bindgen as a library plus
  doxygen-bindgen for the comments; not part of the main build). Do not edit
  `sys.rs` or mirror a libretro struct by hand; replace the header and
  regenerate. The core's exported functions are not in it: `mod.rs`
