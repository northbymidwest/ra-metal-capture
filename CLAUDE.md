...
cargo xtask libretro update                 # vendor the newest libretro.h and regenerate sys.rs
cargo xtask libretro regen                  # regenerate sys.rs from the vendored header
```
...
- All Metal and AppKit calls go through `objc2-metal`, `objc2-foundation`,
  ...
  generated by `cargo xtask libretro regen` from the vendored
  `third_party/libretro/libretro.h` (RetroArch at the commit in
  `third_party/libretro/COMMIT`). The `xtask/` crate is a workspace member
  (aliased to `cargo xtask` in `.cargo/config.toml`; a plain `cargo build`
  never builds it, since the root package is the default member): bindgen
  as a library,
  doxygen-bindgen for the comments, and `libretro update`, which finds
  the newest RetroArch commit that changed the header, vendors it with
  its COMMIT and README, and regenerates. Do not edit `sys.rs` or mirror
  a libretro struct by hand; run the xtask. The core's exported functions are not in it: `mod.rs`
  resolves them by name through `libloading` into `CoreApi`.
...
into `CoreApi` (typed with `sys`'s callback types), and checks
`retro_api_version`; `Core::system_info` names the core, which is
...
empty by default). The callback also records the defaults a core declares
