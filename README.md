...
[RetroArch image mode](#retroarch-image-mode). Either way the result
opens in Xcode's GPU debugger like any other capture.
...
The default backend renders in this process and needs no RetroArch. A
static image through a preset:
...
`fixtures/sample.png` is the 160x144 fixture tracked in this repository: a
cartoon scene drawn by `scripts/gen-sample.py` within Game Boy Color
limits (8x8 tiles, four colours per tile from at most eight palettes,
three-colour sprites, RGB555), so a Game Boy shader sees the kind of input
it was made for. It is this repository's own work under its licence. Any
PNG (or other accepted image format) works in its place.

A libretro core, hosted in this process with a ROM and a save state:
...
That loads the SameBoy core from RetroArch's cores directory, restores the
state, runs one frame, and writes a one-frame trace of it. Without
`--state` or `--slot` the core runs for `--settle` seconds first and the
next frame is recorded.

With `--backend retroarch` the same requests launch RetroArch.app and
capture its window instead:
...
That launches `/Applications/RetroArch.app` with the SameBoy core, loads
the state, advances one frame, and writes a one-frame trace. Without
`--state` or `--slot` it waits for the game to settle and captures
whatever is on screen. `--image` works there too, through RetroArch's
...
| `--image FILE` | static image rendered through the preset (librashader) or shown in RetroArch's image viewer (retroarch); replaces `--core` and `--rom`; incompatible with `--state`, `--slot`, `--advance` |
...
| `--output PATH` | output `.gputrace` path (required); its directory must exist |
| `--state FILE` | save state to load: restored into the hosted core after the ROM loads (librashader), or copied to a temp states dir as slot 0 (retroarch) |
...
| `--size WxH` | exact output size in pixels (librashader) or window size in points (retroarch) |
| `--scale N` | integer scale of the core's native resolution, in output pixels (librashader) or window points (retroarch) |
| `--fullscreen` | render at the main display's full pixel size (librashader) or launch with `-f` (retroarch) |
| `--settle SECS` | without a save state: `round(SECS * fps)` emulated frames run before recording (librashader, image mode has no warm-up), or seconds to wait before capturing (retroarch); default 5, at most 3600 |
| `--advance N` | frames to run after loading the state; the Nth is the recorded one (librashader) or the capture is armed after N frame advances (retroarch); default 1, min 1; only applies with `--state` or `--slot` |
| `--frames N` | consecutive emulated frames to record with the frame count advancing (librashader) or frame boundaries to capture (retroarch); default 1 |
| `--app PATH` | `.app` bundle or its binary; default `/Applications/RetroArch.app`; RetroArch backend only |
| `--cmd-port PORT` | UDP port for RetroArch's command interface, enabled only for this run; default 55355; RetroArch backend only |
| `--keep-running` | do not close RetroArch afterwards; RetroArch backend only |
| `-v` | print sizes and the core's geometry (librashader), or the command line and run config and pass `-v` to RetroArch (retroarch) |

Without a window flag the output is as large as fits the main display,
aspect preserved. Ctrl-C ends either backend cleanly: the capture is
stopped, a partial bundle is removed, a launched RetroArch is killed, and
the tool exits with status 130.

## How it works

### librashader backend

The default backend renders the image, or the hosted core's frames,
through the preset with the
[librashader](https://github.com/SnowflakePowered/librashader) crate's
Metal runtime inside this process, and writes the trace with Metal's
`MTLCaptureManager`. RetroArch is not launched. The tool re-executes
itself once with `MTL_CAPTURE_ENABLED=1` so Metal offers programmatic
capture. `--frames N` renders N successive frames with librashader's
frame count advancing, so frame-count and history passes see real prior
frames. Nothing is presented to a window, so Xcode shows the trace as a
single frame holding N command buffers, one per rendered frame.
...
texture limit. RetroArch and librashader are different implementations of
the preset format; the librashader trace is of librashader's rendering,
not a pixel-exact stand-in for RetroArch's.

### Cores

`--core` and `--rom` load the libretro core into this process. The core
runs headless: no window, no input (every button reads as released, which
is what makes the run repeatable), no audio. `--state` restores a
RetroArch save state (RetroArch's compressed and container formats are
both read); `--slot N` finds it the way RetroArch names it under
`savestate_directory`. The ROM must carry an extension the core declares
it accepts (SameBoy: gb, gbc, and so on) unless `--skip-extension-check`
is given, so a file of the wrong kind is refused instead of loaded as a
cartridge. Under either backend the core runs on its built-in option
defaults unless `--core-options FILE` names a RetroArch-format options
file (for example the per-core file RetroArch keeps at
`~/Library/Application Support/RetroArch/config/SameBoy/SameBoy.opt`), in
which case it renders with the settings that file holds. `--advance N`
then runs N frames, the last of which is recorded, matching the RetroArch
...
directory are resolved against RetroArch's macOS layout
(`~/Library/Application Support/RetroArch` for cores,
`~/Documents/RetroArch` for states and system files) without reading any
config. Only when one of those is not where the layout says does the tool
parse `--config` (default `retroarch.cfg`) and try the directories it
names. A machine without RetroArch therefore works with a full core path,
or with cores in the default folder.
...
SRAM, so `--state` and `--slot` are unaffected). The only use the tool
makes of your `retroarch.cfg` is to find a bare core name, a `--slot`, or
the system directory when they are not in RetroArch's default places; see
[Cores](#cores). RetroArch runs Vulkan through MoltenVK, which is what
makes the trace Metal.
...
your own states directory for `--slot`. Once the process is capturable
the tool pauses RetroArch over that interface, sends `LOAD_STATE`,
frame-advances `--advance` times, and arms `gpucapture`. A paused
RetroArch only presents on a frame advance, and `gpucapture` needs a few
presents to open and close a frame, so the tool keeps advancing until the
capture reports done. The recorded frame is the one `--advance` advances
past the loaded state; the closing advances happen after it.
...
detects, a Ctrl-C handler kills it on interrupt, and a free-running
capture that does not finish within a timeout kills it too, so no process
is left behind. A capture that fails partway removes the partial bundle.

### RetroArch image mode

`--image FILE` under the RetroArch backend loads a static image through
RetroArch's built-in image viewer core instead of an emulator core and
ROM. The viewer reports the image's own pixel dimensions as its content
geometry, so a 160x144 PNG is treated exactly like a Game Boy frame:
window sizing, integer scale, and shader passes all behave the same as
they would with an emulator core. The viewer presents continuously, so
the same settle flow used without a save state captures it; `--state`,
`--slot`, and `--advance` are not accepted with `--image`.
...
frozen image rather than the animation they would see under a running
core; the librashader backend's `--frames` does not have that limit.
